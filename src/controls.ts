...
import { BaseEntity, IEntity } from "./components/entity";
import { Cell, SparseVoxelCellManager } from "./voxels";

export interface FPSControlsParams {
    scene: THREE.Scene;
    camera: THREE.PerspectiveCamera;
    cells: SparseVoxelCellManager;
}
...
export class FPSControls extends BaseEntity<FPSControlsParams> {
    _cells: SparseVoxelCellManager;
    _radius: number;
    _enabled: boolean;
    _move: {
        forward: boolean;
    ...
    constructor(params: FPSControlsParams) {
        super(params);
        this._cells = params.cells;
    }

    init(params: FPSControlsParams) {
        this._radius = 2;
        this._enabled = false;
        this._move = {
            forward: false,
        ...
        if (hasPointerLock) {
            const lockChange = (event) => {
                if (
                    ...
                    this._enabled = true;
                    this._controls.isLocked = true;
                } else {
                    this._controls.isLocked = false;
                }
            };
    ...
    update(timeInSeconds) {
        ...
        for (let c of cells)
            boxes.push(
                ...c.AsBox3Array(this._controls.getObject().position, 3)
            );
